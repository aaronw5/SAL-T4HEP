...
def main():
    ...
    p.add_argument("--test_model", required=False,
                   help="Path to the saved full model (HDF5 or SavedModel)")
    args = p.parse_args()
    ...
    if args.test_model:
        ...
        model_path = os.path.join(args.save_dir, "best.weights.h5")
    ...
    x = np.load(os.path.join(args.data_dir, x_file))
    y = np.load(os.path.join(args.data_dir, y_file))
    logging.info("Loaded TEST arrays: %s, %s", x_file, y_file)

    x = apply_sorting(x, args.sort_by, args.cluster_R, args.cluster_batch_size)
    logging.info("Applied '%s' sorting to TEST set", args.sort_by)
    ...
    current_gpu_mb, peak_gpu_mb = profile_gpu_memory_during_inference(
        model, x[: args.batch_size]
    )
    logging.info(
        "GPU memory — current: %.1f MB, peak: %.1f MB",
        current_gpu_mb, peak_gpu_mb
    )
    ...
    plt.tight_layout()
    roc_path = os.path.join(args.save_dir, 'roc_curves.png')
