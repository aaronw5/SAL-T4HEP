...
class Linformer1L(nn.Module):
    def __init__(self, in_dim: int, d_model: int, num_heads: int, seq_len: int, proj_dim: int, use_flash: bool = False):
        super().__init__()
        self.embed = nn.Linear(in_dim, d_model, bias=False)
        # Note: The optimized MultiheadLinearAttention is now Batch-First [B, T, D]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: [B,T,D]
        x = self.embed(x)
        # REMOVED: q = x.transpose(0, 1) -> The optimized module expects Batch First
        out, _ = self.attn(x, None, None, need_weights=False)
        # REMOVED: out = out.transpose(0, 1)
        y = self.norm1(x + out)
...
class SALT1L(nn.Module):
    def __init__(self, in_dim: int, d_model: int, num_heads: int, seq_len: int, proj_dim: int, conv_filter_heights=None, vertical_stride: int = 1,
                 cluster_E: bool = True, cluster_F: bool = True, share_EF: bool = False, use_flash: bool = False):
        super().__init__()
...
def main():
    ...
    parser.add_argument("--use_flash_linformer", action="store_true", help="Use Flash SDP for Linformer")
    parser.add_argument("--use_flash_salt", action="store_true", help="Use Flash SDP for SAL-T")
    ...
    models = {
        "linformer_1L": Linformer1L(Fin, D, H, seq_len=T, proj_dim=args.proj_dim, use_flash=args.use_flash_linformer),
        "salt_1L": SALT1L(Fin, D, H, seq_len=T, proj_dim=args.proj_dim, conv_filter_heights=args.conv_filter_heights, vertical_stride=args.vertical_stride, use_flash=args.use_flash_salt),
    ...
    for name, model in models.items():
        ...
        for _ in range(100):
            ...
            times_ns.append(avg_ns)
            peaks_mb.append(peak_mb)
            flops_list.append(flops if flops is not None else np.nan)
        times_ns = np.array(times_ns, dtype=float)
        peaks_mb = np.array(peaks_mb, dtype=float)
        flops_arr = np.array(flops_list, dtype=float)
        # Compute mean ± std; handle NaN FLOPs
        time_mean, time_std = float(np.mean(times_ns)), float(np.std(times_ns))
        mem_mean, mem_std = float(np.mean(peaks_mb)), float(np.std(peaks_mb))
        if np.all(np.isnan(flops_arr)):
            flops_str = "N/A"
        else:
            fa = flops_arr[~np.isnan(flops_arr)]
            flops_str = f"{int(np.mean(fa)):,}±{int(np.std(fa)):,}"
