#!/usr/bin/env python
"""
Train a Transolver (PhysicsAttentionStructuredMesh2D) model on jet datasets,
...
# ---------------------------
def parse_args():
    p = argparse.ArgumentParser(description="Train a Transolver on jet data")
    ...
    p.add_argument("--num_particles", type=int, default=150, help="Ignored for 'jetclass'; use default")
    ...
    p.add_argument("--H", type=int, default=0, help="Grid height; if 0, auto-choose")
    p.add_argument("--W", type=int, default=0, help="Grid width; if 0, auto-choose")
    return p.parse_args()
...
def main():
    ...
    assert H * W == num_particles, f"H*W must equal num_particles; got {H}*{W} != {num_particles}"

    # prepare save directory
    save_dir = os.path.join(args.save_dir, str(num_particles), f"{H}x{W}", args.sort_by)
    trial = 0
    while True:
    ...
    # apply sorting (optional; default 'none')
    ...
    # training schedule (modest default)
    schedule = [
        (128, 200),
        (256, 200),
        (512, 200),
