...
def parse_args():
    p = argparse.ArgumentParser(description="Train a Standard Transformer on jet data with sorting and optional convolution")
    ...
    p.add_argument("--sort_by", choices=["pt","eta","phi","delta_R","kt","cluster"], default="pt")
...
def sort_events_by_cluster(x, R, batch_size):
    ...
    for start in range(0, n_events, batch_size):
        ...
        for i, event in enumerate(batch):
            ...
            pz = pts * np.sinh(etas)
            E  = pts * np.cosh(etas)
            ps = [fj.PseudoJet(float(px[j]), float(py[j]), float(pz[j]), float(E[j])) for j in range(n_particles)]
