...
def parse_args():
    p = argparse.ArgumentParser(description="Train a Linformer on jet data with sorting reuse")
    p.add_argument("--data_dir", required=True, help="Directory containing x and y .npy files")
    p.add_argument("--save_dir", required=True, help="Base directory for outputs")
    p.add_argument("--cluster_E", action="store_true", help="Use clustered projection for keys")
    p.add_argument("--cluster_F", action="store_true", help="Use clustered projection for values")
    p.add_argument("--share_EF", action="store_true", help="Share E and F projection matrices (E=F)")
    ...
    p.add_argument("--proj_dim", type=int, default=4, help="Linformer projection dimension")
    p.add_argument("--val_split", type=float, default=0.2)
    p.add_argument("--num_particles", type=int, required=True)
    p.add_argument("--sort_by", choices=["pt","eta","phi","delta_R","kt","cluster"], default="pt", help="How to sort particles")
...
def sort_events_by_cluster(x, R, batch_size):
    ...
    for start in range(0, n_events, batch_size):
        ...
        for i, event in enumerate(batch):
            ...
            for j, pj in enumerate(ps): pj.set_user_index(j)
...
def main():
    ...
    logging.basicConfig(filename=os.path.join(save_dir, "train.log"), filemode='w', level=logging.INFO,
    ...
    if os.path.exists(sorted_path):
        ...
        if args.sort_by in ["pt","eta","phi","delta_R","kt"]:
            if args.sort_by == "pt": key = x[:,:,0]
            elif args.sort_by == "eta": key = x[:,:,1]
            elif args.sort_by == "phi": key = x[:,:,2]
            elif args.sort_by == "delta_R": key = np.sqrt(x[:,:,1]**2 + x[:,:,2]**2)
            else: key = x[:,:,0] * np.sqrt(x[:,:,1]**2 + x[:,:,2]**2)
            idx = np.argsort(key, axis=1)[:, ::-1]
            x = np.take_along_axis(x, idx[:,:,None], axis=1)
        else:
            logging.info("Clustering sort with R=%s", args.cluster_R)
